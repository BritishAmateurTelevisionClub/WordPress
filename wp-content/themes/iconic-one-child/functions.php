...
function custom_pre_get_posts_query( $q ) {
	...
	if ( ! is_admin() && is_shop() ) {
		...
		$q->set( 'tax_query', array(array(
			...
			'terms' => array( 'country' ), // Don't display products in the knives category on the shop page
...
function conditional_guest_checkout_based_on_product( $value ) {
  ...
  $restrict_ids = array( 76, 78, 37, 77 );
...
$_category_id = 6; //put your category ID here
...
//WOOCOMMERCE CHECKOUT INSERT FIELDS
...
//Additional fields to WooCommerce My Account page
...
//Reorder columnsadd_filter('manage_posts_columns', 'column_order');
...
//Remove columns based on user role
...
function bbPressPosting() {
  ...
  foreach($users as $user) {
      ...
      //echo $user->ID . '   ' . $currentRole . '   ' .$result->paydate;
...
function test() {
  session_start();
  global $wpdb;
 $query = PMXE_Plugin::$session->exportquery;
 $query = $query->query_from .' '. $query->query_where;
  ...
  foreach($results as $index => $user) {
    ...
    $query_str .= (($index == 0) ? '' : ',') . $user->ID;

  }



 $_SESSION['test'] =  $query_str;
...
add_action( 'pmxe_template_header', 'test',1,1);



add_action( 'pmxe_options_header','test',1,1 );
 ...
 function test_1() {
  ...
  if (strpos($_SERVER['REQUEST_URI'],'page=pmxe-admin-export') > 0  || strpos($_SERVER['REQUEST_URI'],'page=pmxe-admin-manage') > 0  ) {
    echo '<div id="generate_avery" style="display:none;" class="input">';
    ...
    margin-top: 0px;" target="_blank" class="button button-primary button-hero wpallexport-large-button " href="'. get_stylesheet_directory_uri() .'/avery_labels.php?user_id='. (empty($_SESSION['test']) ? $query_str : $_SESSION['test']).'">Generate Avery Labels</a></div>';

    echo '</div>';

   echo '<div style="display:none;" id="generate_reminder" class="input">';
    ...
    margin-top: 0px;" target="_blank" class="button button-primary button-hero wpallexport-large-button " href="'. get_stylesheet_directory_uri() .'/generate_pdf.php?user_id='. $_SESSION['test'].'&type=reminder">Generate Reminder Letters</a></div>';
...
jQuery(document).ready(function($) {
   ...
   if($('.wp_all_export_download').length > 0) {

jQuery('#generate_avery').appendTo('.wp_all_export_download').show()

   jQuery('#generate_reminder').appendTo('.wp_all_export_download').show()
...
add_action( 'admin_footer', 'test_1');
...
/*
function session_head() {
if (!session_id()) {
    session_start();
}
}
add_action('wp_head', 'session_head', 1);
*/
...
function change_location() {
  ...
  jQuery('document').ready(function() {
    ...
    setTimeout(function(){
    jQuery('.select2').addClass('disabled');
  }, 100);
  })
}
  </script>
...
class WC_Gateway_BACS_custom extends WC_Gateway_BACS {
	...
	$order = new WC_Order( $order_id );
  	...
  	$order->update_status('payment-ou', __( 'Awaiting BACS payment', 'woocommerce' ));
      	$order->add_action( 'woocommerce_order_status_pending_to_on-hold_notification', array( $this, 'trigger' ), 10, 2 );
      */
...
class WC_Gateway_cod_custom extends WC_Gateway_COD {
    ...
    function process_payment( $order_id ) {
      global $woocommerce;
  		$order = new WC_Order( $order_id );
  		/*
  		$order->update_status('payment-ou', __( 'Awaiting check payment', 'woocommerce' ));
  		$order->reduce_order_stock();
  		$woocommerce->cart->empty_cart();
  		return array(
  			'result' 	=> 'success',
  		...
  		*/
  		if ( $order->get_total() > 0 )
			//$order->update_status( apply_filters( 'woocommerce_cod_process_payment_order_status', $order->has_downloadable_item() ? 'on-hold' : 'pending', $order ), __( 'Payment to be made upon delivery.', 'woocommerce' ) );
