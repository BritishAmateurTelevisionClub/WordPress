...
function ihc_login_form_preview(){
		...
		$meta_arr['ihc_login_custom_css'] = stripslashes($_REQUEST['css']);
...
function ihc_do_delete_woo_ihc_relation(){
	if (!empty($_REQUEST['id'])){
		Ihc_Db::ihc_woo_product_custom_price_delete_item($_REQUEST['id']);
		Ihc_Db::ihc_woo_product_custom_price_lid_product_delete($_REQUEST['id']);
		echo 1;
	}
	die();
}
