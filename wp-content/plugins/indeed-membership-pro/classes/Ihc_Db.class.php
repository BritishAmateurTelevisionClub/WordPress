...
<h4>New Updates will be available!</h4>',
	...
	public static function decrement_coupon($coupon_code=''){
			if ($coupon_code){
					global $wpdb;
					$current = $wpdb->get_var("SELECT submited_coupons_count FROM {$wpdb->prefix}ihc_coupons WHERE code='$coupon_code';");
					if ($current){
							$current--;
							return $wpdb->query("UPDATE {$wpdb->prefix}ihc_coupons SET submited_coupons_count=$current WHERE code='$coupon_code'");
					}
			}
			return FALSE;
	}
