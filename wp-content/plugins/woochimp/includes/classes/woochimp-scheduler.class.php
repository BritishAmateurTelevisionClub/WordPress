        ...
        foreach ($order_ids as $order_id) {
            ...
            if ($order = wc_get_order($order_id)) {
                ...
                try {
                    ...
                    // Get order args
                    $args = $WooChimp->prepare_order_data($order_id);
                    ...
                    // Start the log
                    $WooChimp->log_add(__('Sync started for order ', 'woochimp') . $order_id);

                    // Check if order exists
                    $order_exists = $WooChimp->order_exists($args['store_id'], $args['id']);

                    // Write to log
                    $WooChimp->log_add($order_exists ? __('Order exists.', 'woochimp') : __('Order does not exist yet.', 'woochimp'));

                    // Perform the request
                    $result = $order_exists ? $WooChimp->mailchimp->update_order($args['store_id'], $args['id'], $args) : $WooChimp->mailchimp->create_order($args['store_id'], $args);

                    // Write results to log
                    $WooChimp->log_add($order_exists ? __('Order updated successfully.', 'woochimp') : __('Order created successfully.', 'woochimp'));
                    $WooChimp->log_process_regular_data($args, $result);

                    // Update meta
                    RightPress_WC_Meta::order_update_meta_data($order_id, '_woochimp_ecomm_sync', time());
                }

                catch (Exception $e) {

                    $WooChimp->log_add(__('Order data wasn\'t sent.', 'woochimp'));
                    ...
                    if (preg_match('/.+campaign with the provided ID does not exist in the account for this list+/', $e->getMessage())) {
                        ...
                        try {
                            ...
                            // Write results to log
                            $WooChimp->log_add(__('Order data sent successfully, but campaign id was omitted.', 'woochimp'));
                            $WooChimp->log_process_regular_data($args, $result);
                            ...
                            $WooChimp->log_add(__('Order data wasn\'t sent even after omitting campaign id.', 'woochimp'));
                            $WooChimp->log_process_exception($ex);
                            RightPress_WC_Meta::order_update_meta_data($order_id, '_woochimp_ecomm_sync', 'error');
                            continue;
                        }
                    }
                    else {
                        $WooChimp->log_process_exception($e);
                        RightPress_WC_Meta::order_update_meta_data($order_id, '_woochimp_ecomm_sync', 'error');
                    }
