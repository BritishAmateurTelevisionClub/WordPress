...
if (!class_exists('WooChimp')) {
    ...
    class WooChimp
    {
        ...
        {
            // Load helper class
            include_once WOOCHIMP_PLUGIN_PATH . 'includes/classes/libraries/rightpress-helper.class.php';
            include_once WOOCHIMP_PLUGIN_PATH . 'includes/classes/libraries/rightpress-wc-meta.class.php';
            include_once WOOCHIMP_PLUGIN_PATH . 'includes/classes/libraries/rightpress-wc-legacy.class.php';
            ...
            // Note: If order_ properties are added here, appropriate getter must exist in RightPress_WC_Legacy
        ...
        {
            $settings_link = '<a href="http://url.rightpress.net/support-site" target="_blank">'.__('Support', 'woochimp').'</a>';
        ...
        public function prepare_order_data($order_id)
        {
            ...
            $order_customer_id = RightPress_WC_Legacy::order_get_customer_id($order);
            ...
            $order_billing_first_name   = RightPress_WC_Legacy::order_get_billing_first_name($order);
            $order_billing_last_name    = RightPress_WC_Legacy::order_get_billing_last_name($order);

            $customer_details = array(
                'id'            => $customer_id,
                //'email_address' => $customer_email,
                'first_name'    => !empty($order_billing_first_name) ? $order_billing_first_name : RightPress_WC_Legacy::order_get_shipping_first_name($order),
                'last_name'     => !empty($order_billing_last_name) ? $order_billing_last_name : RightPress_WC_Legacy::order_get_shipping_last_name($order),
            ...
            $order_details = array(
                ...
                'currency_code'        => RightPress_WC_Legacy::order_get_currency($order),
                'order_total'          => floatval(RightPress_WC_Legacy::order_get_total($order)),
            ...
            foreach ($items as $item_key => $item) {
                ...
                // Get item name
                $item_name = RightPress_WC_Legacy::order_item_get_name($item);

                // Load actual product
                $product = RightPress_WC_Legacy::order_item_get_product($item, $order);
                $variation_id = RightPress_WC_Legacy::product_get_id($product);
                $product_id = $product->is_type('variation') ? RightPress_WC_Legacy::product_variation_get_parent_id($product) : $variation_id;
                ...
                $order_details['lines'][] = array(
                    ...
                    'quantity'           => intval(RightPress_WC_Legacy::order_item_get_quantity($item)),
                    'price'              => RightPress_WC_Legacy::order_item_get_total($item), // $product->get_price() doesn't fit here because of possible discounts/addons
        ...
        public static function get_mc_id($meta_field, $order_id)
        {
            if (in_array($meta_field, array('woochimp_mc_cid', 'woochimp_mc_eid'))) {
                ...
                $old_mc_id = RightPress_WC_Meta::order_get_meta($order_id, $meta_field, true);
                $new_mc_id = RightPress_WC_Meta::order_get_meta($order_id, '_' . $meta_field, true);
        ...
        public function on_placed($order_id)
        {
            ...
            if (class_exists('WC_Order') && $this->load_mailchimp()) {
                ...
                $subscribe_on_placed = RightPress_WC_Meta::order_get_meta($order_id, 'woochimp_subscribe_on_placed', true);
        ...
        public function on_completed($order_id)
        {
            ...
            if (class_exists('WC_Order') && $this->load_mailchimp()) {
                ...
                // Do we need to subscribe user on completed order or payment?
                $subscribe_on_completed = RightPress_WC_Meta::order_get_meta($order_id, 'woochimp_subscribe_on_completed', true);
                $subscribe_on_payment = RightPress_WC_Meta::order_get_meta($order_id, 'woochimp_subscribe_on_payment', true);
                ...
                try {
                    ...
                    RightPress_WC_Meta::order_update_meta_data($order_id, '_woochimp_ecomm_sent', 1);
        ...
        {
            $woochimp_subscribed_auto = RightPress_WC_Meta::order_get_meta($order_id, '_woochimp_subscribed_auto', true);
            $woochimp_subscribed_checkbox = RightPress_WC_Meta::order_get_meta($order_id, '_woochimp_subscribed_checkbox', true);
        ...
        {
            $woochimp_new_order = RightPress_WC_Meta::order_get_meta($order_id, '_woochimp_new_order', true);
        ...
        {
            $woochimp_ecomm_sent = RightPress_WC_Meta::order_get_meta($order_id, '_woochimp_ecomm_sent', true);
        ...
        public function get_checkout_data($order_id)
        {
            ...
            foreach (array('woochimp_mc_cid', 'woochimp_mc_eid') as $mc_id) {
                ...
                if (isset($_COOKIE[$mc_id])) {
                    RightPress_WC_Meta::order_add_meta_data($order_id, '_' . $mc_id, $_COOKIE[$mc_id], true);
                }

                // Or use a backup plan with hidden values
                else if (isset($_POST['woochimp_data'][$mc_id])) {
                    RightPress_WC_Meta::order_add_meta_data($order_id, '_' . $mc_id, $_POST['woochimp_data'][$mc_id], true);
                }
            }

            // Save groups data posted on checkout
            if (isset($_POST['woochimp_data']['groups'])) {
                RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_groups', $_POST['woochimp_data']['groups'], true);
            }
        ...
        public function new_order($order_id, $admin_order = false)
        {
            ...
            if ($this->checkout_auto_is_active()) {
                ...
                if ($this->opt['woochimp_checkout_auto_subscribe_on'] == '2') {
                    RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_on_completed', 'auto', true);
                }

                // Subscribe on payment received
                else if ($this->opt['woochimp_checkout_auto_subscribe_on'] == '3') {
                    RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_on_payment', 'auto', true);
                }
                ...
                // Subscribe on order placed
                    ...
                    // Otherwise postpone for later
                        ...
                        RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_on_placed', 'auto', true);
                    }
                }
            }
            ...
            if ($this->checkout_checkbox_is_active() && $admin_order === false) {
                ...
                if ($this->opt['woochimp_checkout_checkbox_subscribe_on'] == '2') {
                    RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_on_completed', 'checkbox', true);
                }

                // Subscribe on payment received
                else if ($this->opt['woochimp_checkout_checkbox_subscribe_on'] == '3') {
                    RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_on_payment', 'checkbox', true);
                }

                // Subscribe on order placed
                else {
                    RightPress_WC_Meta::order_add_meta_data($order_id, 'woochimp_subscribe_on_placed', 'checkbox', true);
                }
            }

            // Mark this order as processed
            RightPress_WC_Meta::order_update_meta_data($order_id, '_woochimp_new_order', 1);
        }
            ...
            if (isset($this->opt[$sets_field]) && is_array($this->opt[$sets_field])) {
                foreach ($this->opt[$sets_field] as $set) {
                    ...
                    if ($proceed_subscription) {
                        ...
                        $posted_groups = RightPress_WC_Meta::order_get_meta($order_id, 'woochimp_subscribe_groups', true);
                        ...
                        foreach ($set['fields'] as $custom_field) {
                            if (preg_match('/^order_user_id/', $custom_field['name'])) {
                                ...
                                $method_name = 'order_get_' . $real_field_key;
                                ...
                                if (preg_match('/_state$|_country$/', $real_field_key)) {
                                    ...
                                    $value = RightPress_WC_Legacy::$method_name($order);
                                }
                                ...
                                $custom_order_field = RightPress_WC_Meta::order_get_meta($order_id, $custom_field['value'], true);
                        ...
                        if ($this->subscribe($set['list'], $email, $subscribe_groups, $custom_fields, $user_id, $double_optin) !== false) {
                            RightPress_WC_Meta::order_update_meta_data($order_id, '_woochimp_subscribed_' . $sets_type, 1);
                        }
                    }
        ...
        public function conditions_check($set, $sets_type, $order, $user_meta, $user_id, $is_cart = false)
        {
            ...
            if (is_object($order)) {
                $items = $order->get_items();
                $total = RightPress_WC_Legacy::order_get_total($order);
            ...
            // Products
                ...
                if ($set['condition']['value']['operator'] == 'contains') {
                    foreach ($items as $item) {
                        ...
                        if (RightPress_Helper::wc_version_gte('3.0') && $items_are_order_items) {
            ...
            // Custom field
                ...
                if ($custom_field_value == null) {
                    $order_meta = RightPress_WC_Meta::order_get_meta($order_id, $set['condition']['value']['key'], true);

                    if ($order_meta == '') {
                        $order_meta = RightPress_WC_Meta::order_get_meta($order_id, '_'.$set['condition']['value']['key'], true);
                    }
        ...
        public function maybe_replace_location_code($field_key, $order)
        {
            ...
            $country_code = ($field_type === 'shipping' ? RightPress_WC_Legacy::order_get_shipping_country($order) : RightPress_WC_Legacy::order_get_billing_country($order));

            // Maybe get state code
            if (preg_match('/_state$/', $field_key)) {
                $state_code = ($field_type === 'shipping' ? RightPress_WC_Legacy::order_get_shipping_state($order) : RightPress_WC_Legacy::order_get_billing_state($order));
