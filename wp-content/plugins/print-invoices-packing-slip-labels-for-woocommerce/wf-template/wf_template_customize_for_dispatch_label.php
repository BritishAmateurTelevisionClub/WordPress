...
if(isset($_GET['theme']) && !empty($_GET['theme']))
{
	...
	$dispatch_label_data = $_GET['theme'];

}
else
{
	if(get_option('wf_dispatch_label_active_keys') != '')
	{
		$customize_data = get_option(get_option('wf_dispatch_label_active_keys')); //active 
		$main_data_value = eh_theme_value_assign(get_option('wf_dispatch_label_active_keys'));
		$dispatch_label_data = get_option('wf_dispatch_label_active_keys');
	}
	else
	{
...
$default_active_value = get_option('wf_dispatch_label_active_value');
...
if(isset($_POST['logo_save']))
{
	...
	$main_data_array[85] = !empty($_POST['return_address_display']) ? 'yes' : 'no';
	$main_data_array [83] = $_POST['return_title_align'];
	$main_data_array[84] = !empty($_POST['return_address_title']) ? $_POST['return_address_title'] : $default_active_array[84];
	$main_data_array[88] = !empty($_POST['wf_fee_text']) ? stripslashes($_POST['wf_fee_text']) : $default_active_array[88];
...
function eh_data_save_customize($acive_template,$my_main_data)
{
	...
	if(isset($_GET['theme']) && !empty($_GET['theme']))
	{
		...
		else
		{
			...
			for ($f=1; get_option('wf_dispatch_template_'.$f) !='' ;$f++)
			{
				$i +=1;
			}
			update_option('wf_dispatch_template_'.$i,get_option($_GET['theme']));
			update_option('wf_dispatch_template_'.$i.'value',$my_main_data);
			update_option('wf_dispatch_template_'.$i.'custom', 'yes');
			update_option('wf_dispatch_template_'.$i.'from', $_GET['theme']);
			update_option('wf_dispatch_label_active_keys','wf_dispatch_template_'.$i);
			wp_redirect(admin_url('admin.php?page=wf_template_customize_for_invoice&theme=wf_dispatch_template_'.$i));
		}
	}
	else
	{
		if(get_option($acive_template.'custom') === 'yes')
		{
			...
			update_option('wf_dispatch_label_active_keys',$acive_template);
		}
		else
		{
			for ($f=1; get_option('wf_dispatch_template_'.$f) !='';$f++)
			...
			update_option('wf_dispatch_template_'.$i,get_option($acive_template));
			update_option('wf_dispatch_template_'.$i.'value',$my_main_data);
			update_option('wf_dispatch_template_'.$i.'custom', 'yes');
			update_option('wf_dispatch_template_'.$i.'from', $acive_template);
			update_option('wf_dispatch_label_active_keys','wf_dispatch_template_'.$i);
			wp_redirect(admin_url('admin.php?page=wf_template_customize_for_invoice&theme=wf_dispatch_template_'.$i));
		}
...
function eh_theme_value_assign($given_template)
{
	...
	{
		return get_option('wf_dispatch_label_active_value');
	}
...
<script>
    ...
    $(document).on('mouseenter', '.x_content', function () {
        ...
        $(this).find(":button").hide();
            ...
            <ul class="subsubsub">
                <li><a style="color: #0073aa;" href="<?php echo admin_url('admin.php?page=wf_woocommerce_packing_list&tab=dispatch_label'); ?>" class=""><?php _e('Settings','wf-woocommerce-packing-list'); ?></a> | </li>
                <li><a href="<?php echo admin_url('admin.php?page=wf_template_customize_for_invoice&themeselection=dispatch_label&theme=').get_option('wf_dispatch_label_active_keys'); ?>" class="current"><?php _e('Customize','wf-woocommerce-packing-list'); ?></a></li>
            </ul>
							...
							<div class="x_panel">
										<div class="x_content" >
											...
											<div class="accordion"  style="min-height: 28cm;" id="my_new_invoice" ><?php 
												...
												$customize_data = str_replace('[fee text]', $main_data_array[88], $customize_data);
												...
												if($this->wf_pklist_add_sku === 'Yes') { 
													$customize_data = str_replace('[table tbody content value]','<tr><td class="qty" style="text-align:unset;">Red_Ball</td><td class="desc" style="text-align:unset;">Jumbing LED Light Wall Ball</td><td class="unit" style="text-align:unset;">5</td><td class="total" style="text-align:unset;">$100.00</td>
														<td class="total" style="text-align:unset;">$500.00</td>
														</tr>', $customize_data);
												}else
												{
													$customize_data = str_replace('[table tbody content value]','<tr><td class="desc" style="text-align:unset;" colspan="2">Jumbing LED Light Wall Ball</td><td class="unit" style="text-align:unset;">5</td><td class="total" style="text-align:unset;">$100.00</td><td class="total" style="text-align:unset;">$500.00</td></tr>', $customize_data);
												}
												// $customize_data = str_replace('[invoice barcode data]','Barcode', $customize_data);
												$customize_data = str_replace('[invoice return policy data]','30 Days mony back gurentee', $customize_data);
												...
												// error_log(print_r($main_data_array[84],true));

												if($main_data_array[85] === 'yes' ){
													$customize_data = str_replace('[return address display]', '', $customize_data);
													$customize_data = str_replace('[return title]',$main_data_array[84] , $customize_data);
													$customize_data = str_replace('[return title font]','font-size:16px;', $customize_data);
													$customize_data = str_replace('[return address font]','font-size:16px;', $customize_data);
													$customize_data = str_replace('[return text align]','text-align:'. $main_data_array[83], $customize_data);
													$customize_data = str_replace('[return address]','Name<br>Company name<br>Address1<br>Address2<br>State<br>Country', $customize_data);
												}else{
													$customize_data = str_replace('[return address display]', 'display:none;', $customize_data);
												}
								...
								<div class="col-md-4 col-sm-4 col-xs-12">
									<div class="x_panel">
										<div class="x_title">
											<h2><i class="fa fa-align-left"></i><?php _e('Field Attributes', 'wf-woocommerce-packing-list'); ?><small><?php echo 'Dispatch Label '.substr($dispatch_label_data, -1); ?></small>  </h2>
											...
											<div id="collapseTwo1" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
												<div class="panel-body">
													<div>
														...
														<div class="input-group input-group-sm">
															<label class="input-group-addon" for="logo_extra_details"><?php _e('Tag Line', 'wf-woocommerce-packing-list'); ?></label>
															<textarea class="form-control" id="logo_extra_details" name="logo_extra_details" placeholder="Tag Line" ><?php if(!empty($main_data_array[80])){ echo str_replace('-*-','|',$main_data_array[80]);} ?></textarea>
														</div>
														<div class="input-group input-group-sm">
										...
										<div class="panel">
											<div class="panel-heading  clickable" >
												...
												<h4 class="panel-title collapsed"  role="tab" id="headingFour1" data-toggle="collapse" data-parent="#accordion1" data-target="#collapseFour1" aria-expanded="false" aria-controls="collapseFour"><i class="iconPM fa fa-angle-double-down" aria-hidden="true"></i><?php _e('Dispatch Date', 'wf-woocommerce-packing-list'); ?></h4>
											</div>
											...
											<div class="panel">
												...
												<div class="panel">
													<div class="panel-heading  clickable" >
														<label class="switch pull-right ">
															<input type="checkbox" value="return_address" id="return_address_display" name="return_address_display"<?php echo $main_data_array[85] === 'no' ? '' : 'checked'; ?> />  <div class="slider round"></div>
														</label>
														<h4 class="panel-title collapsed" role="tab" id="headingFive1" data-toggle="collapse" data-parent="#accordion1" data-target="#collapse200" aria-expanded="false" aria-controls="collapseFive"><i class="iconPM fa fa-angle-double-down" aria-hidden="true"></i><?php _e('Return Address', 'wf-woocommerce-packing-list'); ?></h4>
													</div>

													<div id="collapse200" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingFive">
														<div class="panel-body">
															<div>
																<div class="input-group input-group-sm">
																	<label class="input-group-addon" for="return_address_title"><?php _e('Title', 'wf-woocommerce-packing-list'); ?></label>
																		<input class="form-control" id="return_address_title" name="return_address_title" placeholder="Return Address Title" type="text" value="<?php echo $main_data_array[84]; ?>">
																</div>

																<div class="input-group input-group-sm">
																	<label class="input-group-addon" for="return_title_align"><?php _e('Text Align', 'wf-woocommerce-packing-list'); ?></label>
																	<select class="form-control" id="return_title_align" name="return_title_align" ><?php	 
																		if($main_data_array[83] == 'right') 
																		{
																			echo "<option value='right' selected='true' >right</option> <option value='left'>left</option>";
																		}
																		else
																		{
																			echo "<option value='left' selected='true'>left</option><option value='right' >right</option> ";
																		}
																	?>
													...
													<div id="collapse151" class="panel-collapse collapse" role="tabpanel" aria-labelledby="heading15">
														<div class="panel-body">
															<div>
																...
																<div class="input-group input-group-sm">
																	<label class="input-group-addon" for="wf_tt_text"><?php _e('Text', 'wf-woocommerce-packing-list'); ?></label>
																	<input class="form-control" id="wf_fee_text" name="wf_fee_text" placeholder="Fee" type="Total fee text" value="<?php echo $main_data_array[88]; ?>">
																</div>
																...
																<div class="input-group input-group-sm">
																	<label class="input-group-addon" for="wf_paym_text"><?php _e('Text', 'wf-woocommerce-packing-list'); ?>Text</label>
