...
{
	if(get_option('wf_packing_slip_active_key'))
	{
		$customize_data = get_option(get_option('wf_packing_slip_active_key')); //active 
		$main_data_value = eh_theme_value_assign(get_option('wf_packing_slip_active_key'));
		$invoice_data = get_option('wf_packing_slip_active_key');
	}
	else
	{
...
$default_active_value = get_option('wf_packing_slip_active_value');
...
function eh_data_save_customize($acive_template,$my_main_data)
{
	...
	if(isset($_GET['theme']) && !empty($_GET['theme']))
	{
		...
		else
		{
			...
			for ($f=1; get_option('wf_packing_slip_template_'.$f) !='' ;$f++)
			{
				$i +=1;
			}
			update_option('wf_packing_slip_template_'.$i,get_option($_GET['theme']));
			update_option('wf_packing_slip_template_'.$i.'value',$my_main_data);
			update_option('wf_packing_slip_template_'.$i.'custom', 'yes');
			update_option('wf_packing_slip_template_'.$i.'from', $_GET['theme']);
			update_option('wf_packing_slip_active_key','wf_packing_slip_template_'.$i);
			wp_redirect(admin_url('admin.php?page=wf_template_customize_for_invoice&themeselection=packing_slip&theme=wf_packing_slip_template_'.$i));
		}
	}
	else
	{
		if(get_option($acive_template.'custom') === 'yes')
		{
			...
			update_option('wf_packing_slip_active_key',$acive_template);
		}
		else
		{
			for ($f=1; get_option('wf_packing_slip_template_'.$f) !='';$f++)
			...
			update_option('wf_packing_slip_template_'.$i,get_option($acive_template));
			update_option('wf_packing_slip_template_'.$i.'value',$my_main_data);
			update_option('wf_packing_slip_template_'.$i.'custom', 'yes');
			update_option('wf_packing_slip_template_'.$i.'from', $acive_template);
			update_option('wf_packing_slip_active_key','wf_packing_slip_template_'.$i);
			wp_redirect(admin_url('admin.php?page=wf_template_customize_for_invoice&&themeselection=packing_slip&theme=wf_packing_slip_template_'.$i));
		}
...
function eh_theme_value_assign($given_template)
{
	...
	{
		return get_option('wf_packing_slip_active_value');
	}
...
<ul class="subsubsub">
                <li><a href="<?php echo admin_url('admin.php?page=wf_woocommerce_packing_list&tab=packing_slip'); ?>" class="current"><?php _e('Settings','wf-woocommerce-packing-list'); ?></a> | </li>
                <li><a style="color: #0073aa;" href="<?php echo admin_url('admin.php?page=wf_template_customize_for_invoice&themeselection=packing_slip&theme=').get_option('wf_packing_slip_active_key'); ?>" class=""><?php _e('Customize','wf-woocommerce-packing-list'); ?></a></li>
            </ul>
						...
						<div class="col-md-8 col-sm-8 col-xs-12" style="background: white;padding-top: 10px;">
							<div class="x_panel">
								...
								<div class="col-md-4 col-sm-4 col-xs-12">
									<div class="x_panel">
										<div class="x_title">
											<h2><i class="fa fa-align-left"></i><?php _e('Field Attributes', 'wf-woocommerce-packing-list'); ?><small><?php echo 'Pack Slip '.substr($invoice_data, -1); ?></small>  </h2>
