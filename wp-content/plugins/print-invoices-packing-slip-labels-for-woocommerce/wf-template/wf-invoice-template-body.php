...
$main_data = str_replace("[invoice main height and width]", 'width:96vw;page-break-inside:always;',$main_data);
...
//-------------------------------------------------
	...
	$main_data = str_replace('[Signature label]', '', $main_data);
	$main_data = str_replace('signature_text_option_style_data','display:none;', $main_data);
	$main_data = str_replace('signature_img_option_style_data','display:none;', $main_data);
...
if(get_option('woocommerce_wf_packinglist_rtl_settings_enable') == 'Yes')
{
	$main_data 		= str_replace('[rtl class]','rtl_css', $main_data);
}
else
{
	$main_data 		= str_replace('[rtl class]',' ', $main_data);
}


if ($main_data_array[94] === 'no')
	{$main_data 		= str_replace('[order number switch]','display:none !important;', $main_data);}
else
{
...
$main_data = str_replace('[wffootor style]','display:none;width: 100%;',$main_data);
	...
	else
	{
		...
		$main_data 		= str_replace('[product label text]', $main_data_array[70], $main_data); 
			...
			$main_data 		= str_replace('[table quantity text]', $main_data_array[71], $main_data);
                        $main_data 		= str_replace('[table price text]', $default_active_array[88], $main_data);
	...
	if (get_option('woocommerce_calc_shipping') === 'yes'){
		...
		if(!empty($Shippingdetials)){
			...
			$Shipping = $order->get_shipping_to_display();
                ...
                $Shipping = apply_filters('wf_alter_invoice_shipping_method_name', $Shipping , $order);
	...
	if(wc_price((WC()->version < '2.7.0') ? $order->order_total : get_post_meta( $order_id, '_order_total', true ),array('currency'=>$user_currency)))
	{
		...
		if(!empty($refund_data_array))
		{
			foreach ($refund_data_array as  $refund) {
				...
				$refund_amount -=(float) get_post_meta($refund_id,'_order_total',true);
			}
