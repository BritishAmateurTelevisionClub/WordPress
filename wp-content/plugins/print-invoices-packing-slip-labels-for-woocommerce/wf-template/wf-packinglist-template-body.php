...
$ord_date               = (WC()->version < '2.7.0') ? $order->order_date :  $order->get_date_created();
$order_date             = date($main_data_array[16], strtotime($ord_date));
...
$order_shipping = (WC()->version < '2.7.0') ? $order->shipping_method : $order->get_shipping_method();
...
if (get_post_meta($order_id, '_tracking_provider', true) || $order_shipping  ){
	...
	else
	{
		...
		$main_data 		= str_replace('[wf tp position set]','text-align:'.$main_data_array[56].' !important;', $main_data);
		 ...
		 // here ideally shipping value should be printed 
                $main_data 	= str_replace('[tracking data]', $order_shipping, $main_data); 
...
if(get_option('wf_datamatrix_info_for_packinglist') === 'Yes' && $invoice_number != '') 
	{
