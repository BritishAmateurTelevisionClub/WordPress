 ...
 * @version    $Id: Document.php 862 2016-06-16 10:35:15Z jan.slabon $
 */
    ...
    implements SplObserver, SetaPDF_Core_WriteInterface, SetaPDF_Core_Type_Owner
{
     ...
     * @var integer
     */
    protected $_update = self::SAVE_METHOD_REWRITE;
    ...
    )
    {
        ...
        $instance->_update = self::SAVE_METHOD_UPDATE;
    ...
    public function setPdfVersion($pdfVersion)
    {
        ...
        if ($this->_update === self::SAVE_METHOD_UPDATE) {
    ...
    public function addIndirectObjectReferenceWritten(SetaPDF_Core_Type_IndirectObjectInterface $indirectObject)
    {
        ...
        if (
            ...
            && (
                $this->_update <= self::SAVE_METHOD_REWRITE
                ||
     ...
     *
     * @param boolean|string $update Update or rewrite the document
     * @return SetaPDF_Core_Document
    ...
    {
        if ($update !== self::SAVE_METHOD_UPDATE &&
            $update !== self::SAVE_METHOD_REWRITE &&
            $update !== self::SAVE_METHOD_REWRITE_ALL
        ) {
            $update = $update ? self::SAVE_METHOD_UPDATE : self::SAVE_METHOD_REWRITE;
        }
        ...
        if (self::STATE_SAVED === $this->_state && $update !== self::SAVE_METHOD_UPDATE) {
        ...
        if (
            ...
            $update = ($update <= self::SAVE_METHOD_REWRITE) ? $update : self::SAVE_METHOD_REWRITE;
        ...
        if (
            self::SAVE_METHOD_UPDATE === $update &&
        ...
        if (self::SAVE_METHOD_UPDATE === $update &&
            ...
            $this->_update = ($update <= self::SAVE_METHOD_REWRITE) ? $update : self::SAVE_METHOD_REWRITE;
            ...
            if ($this->_parser instanceof SetaPDF_Core_Parser_Pdf) {
                ...
                if (!$rootExists || $this->_update === self::SAVE_METHOD_REWRITE_ALL) {
    ...
    protected function _writeObject(SetaPDF_Core_Document $document, $objectId, $generation, $cache)
    {
         ...
         * is used.
         */
        if ($this->_compressXref && $this->_update === self::SAVE_METHOD_REWRITE_ALL) {
            if ($object->getOwner() instanceof SetaPDF_Core_Type_ObjectStream
                ...
                return;
            } elseif ($object->ensure() instanceof SetaPDF_Core_Type_ObjectStream
    ...
    protected function _writeCrossReferenceTable()
    {
        ...
        if ($this->_compressXref) {
            ...
            $streamObject = $this->_xref->getCompressed($value, $this->_writer->getPos(), $this->_update === self::SAVE_METHOD_UPDATE);
            ...
            $this->_xref->writeNormal($this->_writer, $this->_update === self::SAVE_METHOD_UPDATE);
    ...
    {
        if ($this->_update <= self::SAVE_METHOD_REWRITE || !$this->_compressXref) {
