...
class SetaPDF_Core_Canvas_GraphicState_Text
{
    ...
    protected function _showText($text)
    {
        ...
        $tx = ($w0 * $this->getFontSize()
                + ($this->getCharacterSpacing() * ($charCount - 1))
    ...
    public function showTextStrings($textStrings)
    {
        ...
        foreach ($textStrings AS $textString) {
            if (is_float($textString)) {
                $tx = ((-$textString / 1000) * $this->getFontSize()
                        + ($this->getCharacterSpacing() * 1)
                        + ($this->getWordSpacing() * 0)
                    ) * $this->getScaling() / 100;
                $ty = 0;
     ...
     */
    private function _getFontBBoxVector($name)
    {
        $font = $this->getFont();
        $fontBBox = $font->getFontBBox();
        if ($font instanceof SetaPDF_Core_Font_Type3) {
            $vector = new SetaPDF_Core_Geometry_Vector(0, $fontBBox[$name] * $this->getFontSize());
            $vector = $vector->multiply($font->getFontMatrix());
            return $vector;
        } else {
            return new SetaPDF_Core_Geometry_Vector(0, $fontBBox[$name] / 1000 * $this->getFontSize());
        }
    }
    ...
    {
        return $this->toUserSpace($this->_getFontBBoxVector('lly'));
    }
    ...
    {
        return $this->toUserSpace($this->_getFontBBoxVector('ury'));
    }
